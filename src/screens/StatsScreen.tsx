...
export default function StatsScreen() {
  ...
  const totalDone = useMemo(() => {
    ...
    habits.forEach((h) =>
      Object.keys(h.history).forEach((k) => h.history[k] && keys.add(k)),
    );
    return keys.size;
  }, [habits]);
      ...
      >
          ...
          {bestStreakAll > 0 && (
            <View style={styles.streakBadge}>
              <Text style={styles.streakBadgeText}>
                🔥 Best streak: {bestStreakAll} ngày
              </Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.streakBig}>
          ...
          <View>
            ...
            <Text style={styles.streakSub}>
              Từ{" "}
              {format(subDays(new Date(), curStreakAll), "d/MM", {
                locale: vi,
              })}
            </Text>
          </View>
        ...
        <View style={styles.weekRow}>
          {weekDays.map((d, i) => (
            ...
            >
              <Text
                style={[styles.wdayLabel, d.isToday && { color: COLORS.c4 }]}
              >
                {d.label}
              </Text>
          ...
          <>
            ...
            <View style={styles.habitStatsCard}>
              {habits.map((h, i) => {
                ...
                const doneCount = Object.values(h.history).filter(
                  Boolean,
                ).length;
                  ...
                  >
                    ...
                    <View style={styles.pbarTrack}>
                      <View
                        style={[
                          styles.pbarFill,
                          { width: `${rate}%`, backgroundColor: palette.main },
                        ]}
                      />
                    </View>
        ...
        <View style={styles.heatmap}>
          {heatmapData.map((count, i) => (
            <View
              ...
              style={[
                styles.heatCell,
                { backgroundColor: heatColors[heatLevel(count)] },
              ]}
            />
          ))}
        </View>
        ...
        <View style={styles.heatLegend}>
          ...
          {[0, 1, 2, 3, 4].map((l) => (
            <View
              key={l}
              style={[styles.legendDot, { backgroundColor: heatColors[l] }]}
            />
          ))}
...
const styles = StyleSheet.create({
  ...
  streakBadgeText: { fontSize: 11, fontWeight: "700", color: COLORS.c2l },
  streakBig: {
    flexDirection: "row",
    alignItems: "center",
    gap: 16,
  ...
  wdayTick: { fontSize: 13, marginTop: 2 },
  habitStatsCard: {
  ...
  heatCell: {
    width: (340 - SPACING.xl * 2 - 3 * 12) / 13,
    aspectRatio: 1,
    borderRadius: 3,
  },
  heatLegend: {
